...
class HindentFormatEditsProvider
  ...
  catchExecError(error: NodeJS.ErrnoException) {
    ...
    if (error.code === "ENOENT") {
      message = `hindent-format: Could not execute ${
        this.command
      }. Try specifying the hindent-format.command setting?`;
    }
  ...
  if (hindentFormatProvider.hindentAvailable) {
    ...
    vscode.commands.registerTextEditorCommand(
      ...
      (
        ...
        let formatted = hindentFormatProvider.formatHindent(
